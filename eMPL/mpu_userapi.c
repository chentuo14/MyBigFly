...
//    board_init(); 
  ...
  if (result) {
	...
	{
		MPL_LOGE("Could not enable_9x_sensor_fusion\n");
	    return 0;
	}
	...
	{
		MPL_LOGE("Could not enable_eMPL_outputs.\n");
	    return 0;
	}
  ...
  if (result) {
	...
	{
		MPL_LOGE("Could not set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL | INV_XYZ_COMPASS).\n");
	    return 0;
	}
#else
  ...
  get_ms(&timestamp);
	...
	{
		MPL_LOGE("Could not load firmware.\n");
	    return 0;
	}
    dmp_set_orientation(
        inv_orientation_matrix_to_scalar(gyro_pdata.orientation));
...
/**************************函数说明********************************************
*函数名称:		uint8_tMPU6050_getDeviceID(void)
*函数功能:	    写入MPU6050 WHO_AM_I  地址0x68
*******************************************************************************/
uint8_t MPU6050_getDeviceID(void) 
{
	uint8_t buffer;
    IICreadBytes(0x68, 0x75, 1, &buffer);
    return buffer;
}

/**************************函数说明********************************************
*函数名称:		int mpu9250_initialize(void)
*函数功能:	    9250初始化
*******************************************************************************/
...
u8 mpu_mpl_get_data(float *pitch,float *roll, float *yaw, vs16 *gyro_x, vs16 *gyro_y)
{
    ...
    if(sensors&INV_XYZ_GYRO)
    {
        inv_build_gyro(gyro,sensor_timestamp);          //把角速度MPL
        mpu_get_temperature(&temperature,&sensor_timestamp);
        inv_build_temp(temperature,sensor_timestamp);   //温度
    }
    ...
    if(sensors&INV_XYZ_ACCEL)
    {
        ...
        accel[1] = (long)accel_short[1];
        accel[2] = (long)accel_short[2];
		inv_build_accel(accel,0,sensor_timestamp);      //把加速度给MPL
    }
    ...
    if (!mpu_get_compass_reg(compass_short, &sensor_timestamp))
    {
        ...
        inv_build_compass(compass,0,sensor_timestamp); //把磁力计给MPL
    }
    inv_execute_on_data();
